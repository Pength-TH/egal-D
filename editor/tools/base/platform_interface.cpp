...
namespace egal
{
	namespace PlatformInterface
	{
		...
		FileIterator* createFileIterator(const char* path, IAllocator& allocator)
		{
			...
			StringUnitl::catString(tmp, "/*");
			auto* iter = _aligned_new(allocator, FileIterator);
		...
		void destroyFileIterator(FileIterator* iterator)
		{
			...
			_delete(*iterator->allocator, iterator);
		}
		...
		bool getNextFile(FileIterator* iterator, FileInfo* info)
		{
			...
			StringUnitl::copyString(info->filename, iterator->ffd.cFileName);
		...
		void destroyProcess(Process& process)
		{
			...
			_delete(process.allocator, &process);
		}


		Process* createProcess(const char* cmd, const char* args, IAllocator& allocator)
		{
			auto* process = _aligned_new(allocator, Process)(allocator);
			...
			char rw_args[1024];
			StringUnitl::copyString(rw_args, args);
		...
		bool getSaveFilename(char* out, int max_size, const char* filter, const char* default_extension)
		{
			...
			if (res) StringUnitl::normalize(tmp, out, max_size);
			return res;
		}
				...
				if (SUCCEEDED(pfd->Show(NULL)))
				{
					...
					if (SUCCEEDED(pfd->GetResult(&psi)))
					{
						...
						if (SUCCEEDED(psi->GetDisplayName(SIGDN_DESKTOPABSOLUTEPARSING, &tmp)))
						{
							...
							*c = '\0';
							if (!StringUnitl::endsWith(out, "/") && !StringUnitl::endsWith(out, "\\") && c - out < max_size - 1)
							{
								*c = '/';
								++c;
								*c = '\0';
		...
		void copyToClipboard(const char* text)
		{
			...
			StringUnitl::copyString(mem, len, text);
		...
		bool makePath(const char* path)
		{
			...
			char* out = tmp;
			const char* in = path;
			while (*in && out - tmp < TlengthOf(tmp) - 1)
			{
				*out = *in == '/' ? '\\' : *in;
				++out;
				++in;
