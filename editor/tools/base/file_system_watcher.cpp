#include "editor/tools/base/file_system_watcher.h"
...
namespace egal
{
	...
	struct FileSystemWatcherTask : public MT::Task
	{
		FileSystemWatcherTask(const char* path,
			...
			, m_watcher(watcher)
		{
			StringUnitl::copyString(m_path, path);
		}
	...
	public:
		...
		{
			m_task = _aligned_new(m_allocator, FileSystemWatcherTask)(path, *this, m_allocator);
		...
		TDelegate<void(const char*)>& getCallback() override { return m_callback; }


		TDelegate<void(const char*)> m_callback;
	...
	{
		FileSystemWatcherPC* watcher = _aligned_new(allocator, FileSystemWatcherPC)(allocator);
		if (!watcher->start(path))
		{
			_delete(allocator, watcher);
			return nullptr;
		}
		return watcher;
	}
	...
	void FileSystemWatcher::destroy(FileSystemWatcher* watcher)
	{
		...
		_delete(pc_watcher->m_allocator, pc_watcher);
	}
	...
	int FileSystemWatcherTask::task()
	{
		...
		StringUnitl::setMemory(&m_overlapped, 0, sizeof(m_overlapped));
