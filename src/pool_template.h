#ifndef __pool_template_h__
#define __pool_template_h__
...
class  Pool
{
	public:
		...
		//-----------------------------------------------------------------------
		Pool (bool managed = false) : mManaged(managed),mNextMarked(false)
		{
			if (mManaged)
			{
				// Unused.
		...
		*/
		...
		{
			if (mManaged)
				_log("It is not allowed to add elements, because the pool manages the creation/destruction."); 
		...
		*/
		inline void lockLatestElement (void)
		{
			...
			else if (mPoolIterator == mReleased.end())
			{

			}
			else if (mPoolIterator == mReleased.begin())
			{
				mNextMarked = true;
			}
		};
