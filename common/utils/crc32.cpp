...
namespace egal
{
	static e_uint32 crc32Table[256] = {
	...
	{
		const e_uint8* c = static_cast<const e_uint8*>(data);
		e_uint32 crc = 0xffffFFFF;
		e_int32 len = length;
		while (len)
		{
	...
	{
		const e_uint8* c = reinterpret_cast<const e_uint8*>(str);
		e_uint32 crc = 0xffffFFFF;
		while (*c)
		{
			crc = (crc >> 8) ^ crc32Table[(crc & 0xFF) ^ *c];
	...
	{
		const e_uint8* c = reinterpret_cast<const e_uint8*>(data);
		e_uint32 crc = ~original_crc;
		while (length)
		{
			crc = (crc >> 8) ^ crc32Table[(crc & 0xFF) ^ *c];
