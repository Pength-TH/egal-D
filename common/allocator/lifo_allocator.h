#ifndef _lifo_allocator_h_
#define _lifo_allocator_h_
#pragma once
...
namespace egal
{
	...
	public:
		...
		{
			e_uint8* new_address = (e_uint8*)m_current;
			ASSERT(new_address + size <= (e_uint8*)m_bucket + m_bucket_size);
		...
		{
			if (!ptr) return;
			size_t last_size = *(size_t*)((e_uint8*)m_current - sizeof(size_t));
			e_uint8* last_mem = (e_uint8*)m_current - last_size - sizeof(size_t);
			ASSERT(last_mem == ptr);
			m_current = ptr;
		}
		...
		void* allocate_aligned(size_t size, size_t align) override
		{
			...
			e_uint8* new_address = (e_uint8*)m_current + padding;
			ASSERT(new_address + size <= (e_uint8*)m_bucket + m_bucket_size);
			m_current = new_address + size + sizeof(size_t);
		...
		void* reallocate_aligned(void* ptr, size_t size, size_t align) override
		{
			...
			size_t last_size_with_padding = *(size_t*)((e_uint8*)m_current - sizeof(size_t));
			e_uint8* last_mem = (e_uint8*)m_current - last_size_with_padding - sizeof(size_t);

			size_t padding = (align - ((uintptr)last_mem % align)) % align;
			e_uint8* new_address = (e_uint8*)last_mem + padding;
			ASSERT(new_address + size <= (e_uint8*)m_bucket + m_bucket_size);
			m_current = new_address + size + sizeof(size_t);
			*(size_t*)(new_address + size) = size + padding;
			ASSERT((uintptr)new_address % align == 0);
