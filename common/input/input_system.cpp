...
namespace egal
{
	...
	struct InputSystemImpl : public InputSystem
	{
		...
		{
			m_mouse_device = _aligned_new(m_allocator, MouseDevice);
		...
		float2 getCursorPosition() const override { return m_cursor_pos; }
		void setCursorPosition(const float2& pos) override { m_cursor_pos = pos; }
...
#define REGISTER_SCANCODE(SCANCODE) \
		lua_manager::createSystemVariable(L, "Engine", "INPUT_SCANCODE_" #SCANCODE, (int)SDL_SCANCODE_##SCANCODE);
...
#define REGISTER_KEYCODE(KEYCODE) \
		lua_manager::createSystemVariable(L, "Engine", "INPUT_KEYCODE_" #KEYCODE, (int)SDLK_##KEYCODE);
