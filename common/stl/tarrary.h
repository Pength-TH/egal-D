...
namespace egal
{
	...
	public:
		...
		explicit TArrary(IAllocator& allocator)
		...
		explicit TArrary(const TArrary& rhs)
		...
		void operator=(const TArrary& rhs)
		{
		...
		const T& at(e_int32 index) const
		{
			ASSERT(index >= 0 && index < m_size);
			return m_data[index];
		}
