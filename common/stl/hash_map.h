...
namespace egal
{
	...
	struct HashFunc
	{
		static e_uint32 get(const Key& key);
	};

	template<>
	struct HashFunc<e_uint64>
	{
		static e_uint32 get(const e_uint64& key)
		{
			e_uint64 tmp = (~key) + (key << 18);
			tmp = tmp ^ (tmp >> 31);
			tmp = tmp * 21;
			tmp = tmp ^ (tmp >> 11);
			tmp = tmp + (tmp << 6);
	...
	template<>
	struct HashFunc<e_int32>
	{
		static e_uint32 get(const e_int32& key)
		{
			e_uint32 x = ((key >> 16) ^ key) * 0x45d9f3b;
			x = ((x >> 16) ^ x) * 0x45d9f3b;
			x = ((x >> 16) ^ x);
			return x;
		}
	...
	{
		static e_uint32 get(const ComponentType& key)
		{
			static_assert(sizeof(e_int32) == sizeof(key.index), "Check this");
			return HashFunc<e_int32>::get(key.index);
		}
	};
	...
	{
		static e_uint32 get(const ComponentHandle& key)
		{
			static_assert(sizeof(e_int32) == sizeof(key.index), "Check this");
			return HashFunc<e_int32>::get(key.index);
		}
	...
	{
		static e_uint32 get(const GameObject& key)
		{
			static_assert(sizeof(e_int32) == sizeof(key.index), "Check this");
			return HashFunc<e_int32>::get(key.index);
		}
	...
	template<>
	struct HashFunc<e_uint32>
	{
		static e_uint32 get(const e_uint32& key)
		{
			e_uint32 x = ((key >> 16) ^ key) * 0x45d9f3b;
			x = ((x >> 16) ^ x) * 0x45d9f3b;
			x = ((x >> 16) ^ x);
	...
	{
		static e_uint32 get(const void* key)
		{
			#ifdef PLATFORM_64
				e_uint64 tmp = (e_uint64)key;
				tmp = (~tmp) + (tmp << 18);
				tmp = tmp ^ (tmp >> 31);
				tmp = tmp * 21;
				tmp = tmp ^ (tmp >> 11);
				...
				return (e_uint32)tmp;
			#else
				size_t x = ((e_int32(key) >> 16) ^ e_int32(key)) * 0x45d9f3b;
				x = ((x >> 16) ^ x) * 0x45d9f3b;
				x = ((x >> 16) ^ x);
				return x;
			#endif
	...
	{
		static e_uint32 get(const char* key)
		{
			e_uint32 result = 0x55555555;
	...
	private:
		...
		void copyTable(node_type* src, const node_type* src_sentinel, size_type ids_count)
		{
			for (size_type i = 0; i < ids_count; i++)
			{
				...
				{
					e_int32 pos = getPosition(n->m_key);
					node_type* new_node = getEmptyNode(pos);
					memcpy(new_node, n, sizeof(node_type));
		...
		{
			return _new(where) node_type(key, val);
		}
		...
		{
			return _new(where) node_type(node);
		}
