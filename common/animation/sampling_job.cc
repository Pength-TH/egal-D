...
#include "common/animation/sampling_job.h"
...
namespace egal
{
	namespace animation
	{
		...
		namespace
		{
			...
			void UpdateKeys(float _time, int _num_soa_tracks, egal::Range<const _Key> _keys,
			...
			void UpdateSoaTranslations(int _num_soa_tracks,
				egal::Range<const TranslationKey> _keys,
  ...
  {                                                                            \
    ...
    _quat.w = cpnt[3];                                                         \
			...
			void UpdateSoaRotations(int _num_soa_tracks,
				egal::Range<const RotationKey> _keys, const int* _interp,
...
#undef DECOMPRESS_SOA_QUAT
			...
			void UpdateSoaScales(int _num_soa_tracks, egal::Range<const ScaleKey> _keys,
				const int* _interp, unsigned char* _outdated,
			...
			outdated_scales_(NULL)
		{
			...
			char* alloc_begin = reinterpret_cast<char*>( g_allocator->allocate_aligned(size, ALIGN_OF(InterpSoaTranslation)));
		...
		SamplingCache::~SamplingCache()
		{
			...
			g_allocator->deallocate_aligned(soa_translations_);
		}
