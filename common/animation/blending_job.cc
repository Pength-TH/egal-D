...
#include "common/animation/blending_job.h"
...
#define BLEND_1ST_PASS(_in, _simd_weight, _out)			\
  {                                                     \
...
#define BLEND_N_PASS(_in, _simd_weight, _out)								\
  {                                                                         \
    /* Blends translation. */                                               \
...
#define ADD_PASS(_in, _simd_weight, _out)									 \
  {                                                                          \
    _out.translation = _out.translation + _in.translation * _simd_weight;    \
...
#define SUB_PASS(_in, _simd_weight, _out)									 \
  {                                                                          \
    _out.translation = _out.translation - _in.translation * _simd_weight;    \
    /* Interpolate quaternion between identity and src.rotation.*/           \
    /* Quaternion sign is fixed up, so that lerp takes the shortest path.*/  \
    ...
    _out.scale = _out.scale * rcp_scale;                                     \
			...
			struct ProcessArgs
			{
					...
					accumulated_weight(0.f)
				{
					...
					assert(ARRAY_SIZE(accumulated_weights) >= num_soa_joints);
				}
					...
					if (layer->joint_weights.begin)
					{
						...
						if (_args->num_passes == 0)
						{
							for (size_t i = 0; i < _args->num_soa_joints; ++i)
							{
								...
								BLEND_1ST_PASS(src, weight, dest);
							}
						}
						else
						{
					...
					else
					{
						...
						if (_args->num_passes == 0)
						{
							for (size_t i = 0; i < _args->num_soa_joints; ++i)
							{
								...
								BLEND_1ST_PASS(src, layer_weight, dest);
							}
						}
						else
						{
							for (size_t i = 0; i < _args->num_soa_joints; ++i)
							{
								...
								_args->accumulated_weights[i] =
									_args->accumulated_weights[i] + layer_weight;
								BLEND_N_PASS(src, layer_weight, dest);
							}
						}
					}
			...
			void BlendBindPose(ProcessArgs* _args)
			{
				...
				if (_args->num_partial_passes == 0)
				{
					...
					if (bp_weight > 0.f)
					{  // The bind-pose is needed if it has a weight.
						if (_args->num_passes == 0)
						{
						...
						else
						{
							...
							for (size_t i = 0; i < _args->num_soa_joints; ++i)
							{
								...
								math::SoaTransform* dest = _args->job.output.begin + i;
								BLEND_N_PASS(src, simd_bp_weight, dest);
							}
						}
					}
				}
				else
				{
					...
					for (size_t i = 0; i < _args->num_soa_joints; ++i)
					{
						...
						BLEND_N_PASS(src, bp_weight, dest);
					}
				}
			}
					...
					if (layer->weight > 0.f)
					{
						...
						else
						{
							...
							for (size_t i = 0; i < _args->num_soa_joints; ++i)
							{
								const math::SoaTransform& src = layer->transform.begin[i];
								math::SoaTransform& dest = _args->job.output.begin[i];
								ADD_PASS(src, layer_weight, dest);
							}
						}
					}
					else if (layer->weight < 0.f)
					{
						...
						else
						{
							...
							for (size_t i = 0; i < _args->num_soa_joints; ++i)
							{
								const math::SoaTransform& src = layer->transform.begin[i];
								math::SoaTransform& dest = _args->job.output.begin[i];
								SUB_PASS(src, layer_weight, dest);
							}
						}
					}
					else
