...
namespace egal
{
	...
	typedef TVector<uint16_t> ControlPointRemap;
	typedef TVector<ControlPointRemap> ControlPointsRemap;
		...
		egal::sample::Mesh* _output_mesh)
	{
		...
		// rejected.
		egal::sample::Mesh::Part& part = _output_mesh->parts[0];
		part.positions.reserve(vertex_count *
			egal::sample::Mesh::Part::kPositionsCpnts);
		part.normals.reserve(vertex_count * egal::sample::Mesh::Part::kNormalsCpnts);
		if (element_tangents)
		{
			part.tangents.reserve(vertex_count *
				egal::sample::Mesh::Part::kTangentsCpnts);
		}
		if (element_uvs)
		{
			part.uvs.reserve(vertex_count * egal::sample::Mesh::Part::kUVsCpnts);
		}
		if (element_colors)
		{
			part.colors.reserve(vertex_count * egal::sample::Mesh::Part::kColorsCpnts);
		}
		...
		for (int p = 0; p < polygon_count; ++p)
		{
			...
			for (int v = 0; v < 3; ++v, ++vertex_id)
			{
				...
				for (size_t r = 0; r < remap.size(); ++r)
				{
					...
					if (!Compare(
						&normal.x,
						&part.normals[to_test * egal::sample::Mesh::Part::kNormalsCpnts],
						egal::sample::Mesh::Part::kNormalsCpnts))
					{
					...
					{
						if (!Compare(&uv.x,
							&part.uvs[to_test * egal::sample::Mesh::Part::kUVsCpnts],
							egal::sample::Mesh::Part::kUVsCpnts))
						{
							continue;  // Next vertex.
						}
					}
					...
					if (element_colors)
					{
						if (!Compare(
							color,
							&part.colors[to_test * egal::sample::Mesh::Part::kColorsCpnts],
							egal::sample::Mesh::Part::kColorsCpnts))
						{
							continue;  // Next vertex.
						}
					}
				...
				else
				{
					...
					uint16_t vertex_index = static_cast<uint16_t>(
						part.positions.size() / egal::sample::Mesh::Part::kPositionsCpnts);
	...
	typedef TVector<SkinMapping> SkinMappings;
	typedef TVector<SkinMappings> VertexSkinMappings;
		...
		egal::sample::Mesh* _output_mesh)
	{
		...
		typedef egal::CStringMap<uint16_t> JointsMap;
		...
		VertexSkinMappings vertex_skin_mappings(*g_allocator);
		...
		for (size_t i = 0; i < vertex_count; ++i)
		{
			...
			max_influences = egal::math::Max(max_influences, inv.size());
	...
	bool LimitInfluences(egal::sample::Mesh& _skinned_mesh, int _limit)
	{
		assert(_skinned_mesh.parts.size() == 1);

		egal::sample::Mesh::Part& in_part = _skinned_mesh.parts.front();
	...
	bool SplitParts(const egal::sample::Mesh& _skinned_mesh,
		egal::sample::Mesh* _partitionned_mesh)
	{
		assert(_skinned_mesh.parts.size() == 1);
		assert(_partitionned_mesh->parts.size() == 0);

		const egal::sample::Mesh::Part& in_part = _skinned_mesh.parts.front();
		const size_t vertex_count = in_part.vertex_count();
		...
		typedef TVector<TVector<size_t>> BuckedVertices;
		BuckedVertices bucked_vertices(*g_allocator);
		...
		{
			const TVector<size_t>& bucket = bucked_vertices[i];
			...
			egal::sample::Mesh::Part& out_part = _partitionned_mesh->parts.back();
			...
			for (size_t j = 0; j < bucket_vertex_count; ++j)
			{
				...
				// Fills positions.
				float* out_pos =
					&out_part.positions[j * egal::sample::Mesh::Part::kPositionsCpnts];
				const float* in_pos =
				...
				// Fills normals.
				float* out_normal =
					&out_part.normals[j * egal::sample::Mesh::Part::kNormalsCpnts];
				...
				if (in_part.uvs.size())
				{
					float* out_uv = &out_part.uvs[j * egal::sample::Mesh::Part::kUVsCpnts];
					const float* in_uv =
						&in_part
						.uvs[bucket_vertex_index * egal::sample::Mesh::Part::kUVsCpnts];
					out_uv[0] = in_uv[0];
					out_uv[1] = in_uv[1];
				}
				// Fills colors.
				...
				{
					uint8_t* out_color =
						&out_part.colors[j * egal::sample::Mesh::Part::kColorsCpnts];
				...
				{
					float* out_tangent =
						&out_part.tangents[j * egal::sample::Mesh::Part::kTangentsCpnts];
		...
		TVector<uint16_t> vertices_remap(*g_allocator);
	...
	// weights equals 1).
	bool StripWeights(egal::sample::Mesh* _mesh)
	{
		for (size_t i = 0; i < _mesh->parts.size(); ++i)
		{
			egal::sample::Mesh::Part& part = _mesh->parts[i];
			...
			{
				const TVector<float> copy = part.joint_weights;
				part.joint_weights.clear();
