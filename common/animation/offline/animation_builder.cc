	...
	namespace animation
	{
		namespace offline
		{
			namespace
			{
					...
					_DestTrack* _dest)
				{
					...
					{  // Copies all keys, and fixes up first and last keys.
						...
						{  // Copies all keys.
							...
							const DestKey key =
							{ _track, prev_time,
							{raw_key.time, raw_key.value} };
							_dest->push_back(key);
							prev_time = raw_key.time;
						}
						if (_src.back().time != _duration)
						{  // Needs a key at t = _duration.
							const DestKey last =
							{ _track, prev_time,
							{_duration, _src.back().value} };
							_dest->push_back(last);
						}
					}
					...
					egal::Range<TranslationKey>* _dest)
				{
					...
					for (size_t i = 0; i < src_count; ++i)
					{
						...
						key.value[0] = egal::math::FloatToHalf(src[i].key.value.x);
						key.value[1] = egal::math::FloatToHalf(src[i].key.value.y);
						key.value[2] = egal::math::FloatToHalf(src[i].key.value.z);
					}
				}
				...
				namespace
				{
					...
					// Compresses quaternion to egal::animation::RotationKey format.
					...
					void CompressQuat(const egal::math::Quaternion& _src,
						egal::animation::RotationKey* _dest)
					{
						// Finds the largest quaternion component.
						const float quat[4] =
						{ _src.x, _src.y, _src.z, _src.w };
			...
			Animation* AnimationBuilder::operator()(const RawAnimation& _input) const
			{
				...
				Animation* animation = (Animation*)g_allocator->allocate(sizeof(Animation));
