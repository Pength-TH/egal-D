...
namespace egal
{
	namespace animation
	{
			...
			int _joint)
		{
			...
			const egal::math::SoaTransform& soa_transform =
				_skeleton.bind_pose()[_joint / 4];

			// Transpose SoA data to AoS.
			egal::math::SimdFloat4 translations[4];
			egal::math::Transpose3x4(&soa_transform.translation.x, translations);
			egal::math::SimdFloat4 rotations[4];
			egal::math::Transpose4x4(&soa_transform.rotation.x, rotations);
			egal::math::SimdFloat4 scales[4];
			egal::math::Transpose3x4(&soa_transform.scale.x, scales);

			// Stores to the Transform object.
			math::Transform bind_pose;
			const int offset = _joint % 4;
			egal::math::Store3PtrU(translations[offset], &bind_pose.translation.x);
			egal::math::StorePtrU(rotations[offset], &bind_pose.rotation.x);
			egal::math::Store3PtrU(scales[offset], &bind_pose.scale.x);
...
// processed).
			...
			JointsIterator* _iterator)
		{
			...
			{  // num_joints > 0, which was tested as pre-conditions.
				start.joint = 0;
			...
			for (; stack_size != 0;)
			{
				...
				if (!properties.begin[top.joint].is_leaf)
				{  // A leaf has no child anyway.
