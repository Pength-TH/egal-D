...
#ifndef _IO_ARCHIVE_H_
#define _IO_ARCHIVE_H_
...
// functions:
// - The intrusive function prototypes are "void Save(egal::io::OArchive*) const"
// and "void Load(egal::io::IArchive*)".
// - The non-intrusive functions allow to work on arrays of objects. They must
// be implemented in egal::io namespace with these prototypes:
...
// helper function egal::io::MakeArray() that is then streamed in or out using
// << and >> archive operators: archive << egal::io::MakeArray(my_array, count);
//
// Versioning can be done using IO_TYPE_VERSION macros. Type version
...
// IO_TYPE_NOT_VERSIONABLE like macros. It can not be re-enabled afterward.
//
// Objects can be assigned a tag using IO_TYPE_TAG macros. A tag allows to
...
// an output archive (egal::io::OArchive). Input archives automatically handle
...
#undef _IO_PRIMITIVE_TYPE
		...
		public:
			...
			void operator>>(_Ty& _ty)
			{
				...
				IF_DEBUG(bool valid = ) internal::Tagger<const _Ty>::Validate(*this);
...
#undef _IO_PRIMITIVE_TYPE
		...
		IO_TYPE_NOT_VERSIONABLE(char)
		IO_TYPE_NOT_VERSIONABLE(int8_t)
		IO_TYPE_NOT_VERSIONABLE(uint8_t)
		IO_TYPE_NOT_VERSIONABLE(int16_t)
		IO_TYPE_NOT_VERSIONABLE(uint16_t)
		IO_TYPE_NOT_VERSIONABLE(int32_t)
		IO_TYPE_NOT_VERSIONABLE(uint32_t)
		IO_TYPE_NOT_VERSIONABLE(int64_t)
		IO_TYPE_NOT_VERSIONABLE(uint64_t)
		IO_TYPE_NOT_VERSIONABLE(bool)
		IO_TYPE_NOT_VERSIONABLE(float)
		...
		// Must be used through egal::io::MakeArray.
		namespace internal
		{
			template <typename _Ty>
			struct Array {
...
#undef _IO_PRIMITIVE_TYPE
		...
		template <typename _Ty>
		INLINE const internal::Array<_Ty> MakeArray(_Ty* _array, size_t _count)
		{
			const internal::Array<_Ty> array = { _array, _count };
			return array;
		}
		template <typename _Ty>
		INLINE const internal::Array<const _Ty> MakeArray(const _Ty* _array,
			size_t _count)
		{
		...
		template <typename _Ty>
		INLINE const internal::Array<_Ty> MakeArray(Range<_Ty> _array)
		{
			const internal::Array<_Ty> array = { _array.begin, _array.Count() };
			return array;
		}
		template <typename _Ty>
		INLINE const internal::Array<const _Ty> MakeArray(Range<const _Ty> _array)
		{
			const internal::Array<const _Ty> array = { _array.begin, _array.Count() };
			return array;
		}
		template <typename _Ty, size_t _count>
		INLINE const internal::Array<_Ty> MakeArray(_Ty(&_array)[_count])
		{
			const internal::Array<_Ty> array = { _array, _count };
			return array;
		}
		template <typename _Ty, size_t _count>
		INLINE const internal::Array<const _Ty> MakeArray(
