...
namespace egal
{
	...
	struct Transform
	{
		...
		Transform(const float3& _pos, const Quaternion& _rot, float _scale)
			: pos(_pos)
			, rot(_rot)
			, scale(_scale)
		{
	...
	ALIGN_BEGIN(16) struct Matrix
	{
		...
		Matrix(const float3& x, const float3& y, const float3& z)
		{
			setXVector(x);
			setYVector(y);
			setZVector(z);
		...
		Matrix(const float3& pos, const Quaternion& rot);
		...
		void decompose(float3& position, Quaternion& rotation, float& scale) const;
		...
		float3 getZVector() const
		{
			return float3(m31, m32, m33);
		}

		float3 getYVector() const
		{
			return float3(m21, m22, m23);
		}

		float3 getXVector() const
		{
			return float3(m11, m12, m13);
		}

		void setXVector(const float3& v)
		{
			m11 = v.x; m12 = v.y; m13 = v.z;
		}

		void setYVector(const float3& v)
		{
			m21 = v.x; m22 = v.y; m23 = v.z;
		}

		void setZVector(const float3& v)
		{
		...
		void lookAt(const float3& eye, const float3& at, const float3& up)
		{
			*this = Matrix::IDENTITY;
			...
			setZVector(f);
			transpose();
			setTranslation(float3(-dotProduct(r, eye), -dotProduct(u, eye), -dotProduct(f, eye)));
		}
		...
		void getTranslation(float3& pos) const
		{
			pos.set(m41, m42, m43);
		}
		...
		void transpose();
		float3 transformPoint(const float3& pos) const;
		float3 transformVector(const float3& pos) const;
