...
#include "runtime/lua/lua_manager.h"
namespace egal
{
	namespace LuaImGui
	{
		...
		{
			char buf[4096];
			auto* name = lua_manager::checkArg<const char*>(L, 1);
			auto* value = lua_manager::checkArg<const char*>(L, 2);
		...
		{
			auto* name = lua_manager::checkArg<const char*>(L, 1);
			float value = lua_manager::checkArg<float>(L, 2);
		...
		{
			int var = lua_manager::checkArg<int>(L, 1);
			ImVec4 v;
			v.x = lua_manager::checkArg<float>(L, 2);
			v.y = lua_manager::checkArg<float>(L, 3);
			v.z = lua_manager::checkArg<float>(L, 4);
			v.w = lua_manager::checkArg<float>(L, 5);
		...
		int PushStyleVar(lua_State* L)
		{
			...
			{
				float v = lua_manager::checkArg<float>(L, 2);
				ImGui::PushStyleVar(var, v);
			}
			return 0;
		}
		...
		{
			int id = lua_manager::checkArg<int>(L, 1);
			ImGui::PushID(id);
			return 0;
		}
		...
		int SetStyleColor(lua_State* L)
		{
			...
			int index = lua_manager::checkArg<int>(L, 1);
			ImVec4 color;
			color.x = lua_manager::checkArg<float>(L, 2);
			color.y = lua_manager::checkArg<float>(L, 3);
			color.z = lua_manager::checkArg<float>(L, 4);
			color.w = lua_manager::checkArg<float>(L, 5);
		...
		int SliderFloat(lua_State* L)
		{
			auto* name = lua_manager::checkArg<const char*>(L, 1);
			float value = lua_manager::checkArg<float>(L, 2);
			float min = lua_manager::checkArg<float>(L, 3);
			float max = lua_manager::checkArg<float>(L, 4);
		...
		{
			auto* text = lua_manager::checkArg<const char*>(L, 1);
			ImGui::Text("%s", text);
			return 0;
		}
		...
		{
			auto* label = lua_manager::checkArg<const char*>(L, 1);
			auto* text = lua_manager::checkArg<const char*>(L, 2);
		...
		int Button(lua_State* L)
		{
			...
			if (lua_gettop(L) > 2)
			{
				size.x = lua_manager::checkArg<float>(L, 2);
				size.y = lua_manager::checkArg<float>(L, 3);
			}
		...
		int CalcTextSize(lua_State* L)
		{
			...
			lua_manager::push(L, size.x);
			lua_manager::push(L, size.y);
			return 2;
		}
		...
		{
			auto* label = lua_manager::checkArg<const char*>(L, 1);
			bool b = lua_manager::checkArg<bool>(L, 2);
		...
		int GetWindowPos(lua_State* L)
		{
			...
			lua_manager::push(L, Vec2(pos.x, pos.y));
			return 1;
		}
		...
		{
			ImVec2 pos;
			pos.x = lua_manager::checkArg<float>(L, 1);
			pos.y = lua_manager::checkArg<float>(L, 2);
		...
		int Selectable(lua_State* L)
		{
			...
			if (lua_gettop(L) > 1)
			{
				selected = lua_manager::checkArg<bool>(L, 2);
			}
		...
		int Begin(lua_State* L)
		{
			...
			if (lua_gettop(L) > 1)
			{
				flags = lua_manager::checkArg<int>(L, 2);
			}
		...
		int SameLine(lua_State* L)
		{
			...
			if (lua_gettop(L) > 0)
			{
				pos_x = lua_manager::checkArg<float>(L, 1);
			}
...
static const e_uint32 SERIALIZED_ENGINE_MAGIC = 0x5f4c454e; // == '_LEN'
...
public:
	...
	{
		log_error("Core Creating engine...");
		...
		if (!fs)
		{
			...
			m_mem_file_device = _aligned_new(m_allocator, FS::MemoryFileDevice)(m_allocator);
			m_resource_file_device = _aligned_new(m_allocator, FS::ResourceFileDevice)(m_allocator);
			m_disk_file_device = _aligned_new(m_allocator, FS::DiskFileDevice)("disk", base_path0, m_allocator);

			m_file_system->mount(m_mem_file_device);
			m_file_system->mount(m_resource_file_device);
			m_file_system->mount(m_disk_file_device);
			bool is_patching = base_path1[0] != 0 && !StringUnitl::equalStrings(base_path0, base_path1);
			if (is_patching)
			{
				m_patch_file_device = _aligned_new(m_allocator, FS::DiskFileDevice)("patch", base_path1, m_allocator);
	...
	static ComponentHandle LUA_createComponent(Universe* universe, Entity entity, const char* type)
	{
		...
		{
			log_error("Lua Script Component %s already exists in entity %s.", type, entity.index);
	...
	struct SetPropertyVisitor : public Reflection::IPropertyVisitor
	{
		...
		void visit(const Reflection::Property<Vec2>& prop) override
		{
			...
			if (lua_istable(L, -1))
			{
				auto v = lua_manager::toType<Vec2>(L, -1);
				InputBlob input_blob(&v, sizeof(v));
				prop.setValue(cmp, -1, input_blob);
			}
		}
		...
		void visit(const Reflection::Property<Vec3>& prop) override
		{
			...
			if (lua_istable(L, -1))
			{
				auto v = lua_manager::toType<Vec3>(L, -1);
				InputBlob input_blob(&v, sizeof(v));
				prop.setValue(cmp, -1, input_blob);
			}
		}
		...
		void visit(const Reflection::Property<Vec4>& prop) override
		{
			...
			if (lua_istable(L, -1))
			{
				auto v = lua_manager::toType<Vec4>(L, -1);
				InputBlob input_blob(&v, sizeof(v));
				prop.setValue(cmp, -1, input_blob);
			}
		}
		...
		{
			if (!equalStrings(property_name, prop.name)) return;
			log_error("Lua Script") << "Property " << prop.name << " has unsupported type";
		}
	...
	{
		auto* ctx = lua_manager::checkArg<Universe*>(L, 2);
		...
		while (lua_next(L, -2) != 0)
		{
			...
			{
				auto pos = lua_manager::toType<Vec3>(L, -1);
				ctx->setPosition(e, pos);
			}
			else if (equalStrings(parameter_name, "rotation"))
			{
				auto rot = lua_manager::toType<Quat>(L, -1);
	...
	{
		auto* engine = lua_manager::checkArg<Engine*>(L, 1);
		auto* universe = lua_manager::checkArg<Universe*>(L, 2);
		...
		struct Callback
		{
			...
			void invoke(FS::IFile& file, bool success)
			{
				if (!success)
				{
					log_error("Engine Failed to open universe %s.", path);
				}
				else
				{
					...
					{
						log_error("Engine Failed to deserialize universe %s.", path);
					}
					else
					{
	...
	{
		Vec3 v = lua_manager::checkArg<Vec3>(L, 1);
		Quat q;
		if (lua_manager::isType<Quat>(L, 2))
		{
			q = lua_manager::checkArg<Quat>(L, 2);
		}
		else
		{
			Vec3 axis = lua_manager::checkArg<Vec3>(L, 2);
	...
	void registerLuaAPI()
	{
		...
		lua_manager::createSystemFunction(m_state, "Engine", "loadUniverse", LUA_loadUniverse);

		#undef REGISTER_FUNCTION

		lua_manager::createSystemFunction(m_state, "Engine", "instantiatePrefab", &LUA_instantiatePrefab);
		lua_manager::createSystemFunction(m_state, "Engine", "createEntityEx", &LUA_createEntityEx);
		lua_manager::createSystemFunction(m_state, "Engine", "multVecQuat", &LUA_multVecQuat);

		lua_newtable(m_state);
		lua_pushvalue(m_state, -1);
		lua_setglobal(m_state, "ImGui");

		lua_manager::createSystemVariable(m_state, "ImGui", "WindowFlags_NoMove", ImGuiWindowFlags_NoMove);
		lua_manager::createSystemVariable(m_state, "ImGui", "WindowFlags_NoCollapse", ImGuiWindowFlags_NoCollapse);
		lua_manager::createSystemVariable(m_state, "ImGui", "WindowFlags_NoInputs", ImGuiWindowFlags_NoInputs);
		lua_manager::createSystemVariable(m_state, "ImGui", "WindowFlags_NoResize", ImGuiWindowFlags_NoResize);
		lua_manager::createSystemVariable(m_state, "ImGui", "WindowFlags_NoTitleBar", ImGuiWindowFlags_NoTitleBar);
		lua_manager::createSystemVariable(m_state, "ImGui", "WindowFlags_NoScrollbar", ImGuiWindowFlags_NoScrollbar);
		lua_manager::createSystemVariable(m_state, "ImGui", "WindowFlags_AlwaysAutoResize", ImGuiWindowFlags_AlwaysAutoResize);
		lua_manager::createSystemVariable(m_state, "ImGui", "Col_FrameBg", ImGuiCol_FrameBg);
		lua_manager::createSystemVariable(m_state, "ImGui", "Col_WindowBg", ImGuiCol_WindowBg);
		lua_manager::createSystemVariable(m_state, "ImGui", "Col_Button", ImGuiCol_Button);
		lua_manager::createSystemVariable(m_state, "ImGui", "Col_ButtonActive", ImGuiCol_ButtonActive);
		lua_manager::createSystemVariable(m_state, "ImGui", "Col_ButtonHovered", ImGuiCol_ButtonHovered);
		lua_manager::createSystemVariable(m_state, "ImGui", "StyleVar_FramePadding", ImGuiStyleVar_FramePadding);
		lua_manager::createSystemVariable(m_state, "ImGui", "StyleVar_IndentSpacing", ImGuiStyleVar_IndentSpacing);
		lua_manager::createSystemVariable(m_state, "ImGui", "StyleVar_ItemSpacing", ImGuiStyleVar_ItemSpacing);
		lua_manager::createSystemVariable(m_state, "ImGui", "StyleVar_ItemInnerSpacing", ImGuiStyleVar_ItemInnerSpacing);
		lua_manager::createSystemVariable(m_state, "ImGui", "StyleVar_WindowPadding", ImGuiStyleVar_WindowPadding);
		...
		LuaImGui::registerCFunction(m_state, "BeginPopup", &lua_manager::wrap<decltype(&ImGui::BeginPopup), &ImGui::BeginPopup>);
		...
		LuaImGui::registerCFunction(m_state, "Columns", &lua_manager::wrap<decltype(&ImGui::Columns), &ImGui::Columns>);
		LuaImGui::registerCFunction(m_state, "DragFloat", &LuaImGui::DragFloat);
		LuaImGui::registerCFunction(m_state, "Dummy", &lua_manager::wrap<decltype(&LuaImGui::Dummy), &LuaImGui::Dummy>);
		LuaImGui::registerCFunction(m_state, "End", &lua_manager::wrap<decltype(&ImGui::End), &ImGui::End>);
		LuaImGui::registerCFunction(m_state, "EndChildFrame", &lua_manager::wrap<decltype(&ImGui::EndChildFrame), &ImGui::EndChildFrame>);
		LuaImGui::registerCFunction(m_state, "EndDock", &lua_manager::wrap<decltype(&ImGui::EndDock), &ImGui::EndDock>);
		LuaImGui::registerCFunction(m_state, "EndPopup", &lua_manager::wrap<decltype(&ImGui::EndPopup), &ImGui::EndPopup>);
		LuaImGui::registerCFunction(m_state, "GetColumnWidth", &lua_manager::wrap<decltype(&ImGui::GetColumnWidth), &ImGui::GetColumnWidth>);
		...
		LuaImGui::registerCFunction(m_state, "Image", &lua_manager::wrap<decltype(&LuaImGui::Image), &LuaImGui::Image>);
		LuaImGui::registerCFunction(m_state, "Indent", &lua_manager::wrap<decltype(&ImGui::Indent), &ImGui::Indent>);
		LuaImGui::registerCFunction(m_state, "InputTextMultiline", &LuaImGui::InputTextMultiline);
		LuaImGui::registerCFunction(m_state, "IsItemHovered", &lua_manager::wrap<decltype(&LuaImGui::IsItemHovered), &LuaImGui::IsItemHovered>);
		LuaImGui::registerCFunction(m_state, "IsMouseClicked", &lua_manager::wrap<decltype(&LuaImGui::IsMouseClicked), &LuaImGui::IsMouseClicked>);
		LuaImGui::registerCFunction(m_state, "IsMouseDown", &lua_manager::wrap<decltype(&LuaImGui::IsMouseDown), &LuaImGui::IsMouseDown>);
		LuaImGui::registerCFunction(m_state, "NewLine", &lua_manager::wrap<decltype(&ImGui::NewLine), &ImGui::NewLine>);
		LuaImGui::registerCFunction(m_state, "NextColumn", &lua_manager::wrap<decltype(&ImGui::NextColumn), &ImGui::NextColumn>);
		LuaImGui::registerCFunction(m_state, "OpenPopup", &lua_manager::wrap<decltype(&ImGui::OpenPopup), &ImGui::OpenPopup>);
		LuaImGui::registerCFunction(m_state, "PopItemWidth", &lua_manager::wrap<decltype(&ImGui::PopItemWidth), &ImGui::PopItemWidth>);
		LuaImGui::registerCFunction(m_state, "PopID", &lua_manager::wrap<decltype(&ImGui::PopID), &ImGui::PopID>);
		LuaImGui::registerCFunction(m_state, "PopStyleColor", &lua_manager::wrap<decltype(&ImGui::PopStyleColor), &ImGui::PopStyleColor>);
		LuaImGui::registerCFunction(m_state, "PopStyleVar", &lua_manager::wrap<decltype(&ImGui::PopStyleVar), &ImGui::PopStyleVar>);
		LuaImGui::registerCFunction(m_state, "PushItemWidth", &lua_manager::wrap<decltype(&ImGui::PushItemWidth), &ImGui::PushItemWidth>);
		...
		LuaImGui::registerCFunction(m_state, "Rect", &lua_manager::wrap<decltype(&LuaImGui::Rect), &LuaImGui::Rect>);
		...
		LuaImGui::registerCFunction(m_state, "SetNextWindowSize", &lua_manager::wrap<decltype(&LuaImGui::SetNextWindowSize), &LuaImGui::SetNextWindowSize>);
		...
		LuaImGui::registerCFunction(m_state, "Unindent", &lua_manager::wrap<decltype(&ImGui::Unindent), &ImGui::Unindent>);
	...
	static int LUA_packageLoader(lua_State* L)
	{
		...
		if (!file)
		{
			log_error("Engine") << "Failed to open file " << tmp;
		...
		{
			log_error("Engine") << "Failed to load package " << tmp << ": " << lua_tostring(L, -1);
		}
	...
	bool hasSupportedSceneVersions(InputBlob& serializer, Universe& ctx)
	{
		...
		for (int i = 0; i < count; ++i)
		{
			...
			{
				log_error("Core") << "Plugin " << scene->getPlugin().getName() << " has incompatible version";
				return false;
			}
		}
		return true;
	...
	static int LUA_instantiatePrefab(lua_State* L)
	{
		...
		auto* universe = lua_manager::checkArg<Universe*>(L, 2);
		Vec3 position = lua_manager::checkArg<Vec3>(L, 3);
		...
		if (!prefab->isReady())
		{
			log_error("Editor Prefab %s is not ready, preload it.", prefab->getPath().c_str());
			return 0;
		}
...
{
	return _aligned_new(allocator, EngineImpl)(base_path0, base_path1, fs, allocator);
}
